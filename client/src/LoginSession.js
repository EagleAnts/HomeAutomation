...
const LoginSession = () => {
  ...
  useEffect( () => {
    const checkLoggedIn = async (setLogHomeFlag, style) => {
      const data = await axios.post(`http://localhost:${process.env.PORT || 5000}/`, {})
          if (data.data === true) {
            style.href = "./css/default.css";
            rsaEncrypt({}, "setUp").then((data=> console.log(data))).catch((err)=>console.log(err));
            console.log("qwerty1")
      ...
      checkLoggedIn(setLogHomeFlag, style).then((data) =>       
      setLoading(false))
  }, []);
